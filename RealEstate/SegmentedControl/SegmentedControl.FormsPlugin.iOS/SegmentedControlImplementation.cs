...
namespace SegmentedControl.FormsPlugin.iOS
{
	...
	public class SegmentedControlRenderer : ViewRenderer<Abstractions.SegmentedControl, UISegmentedControl>
	{
        ...
        void SetBackgroundSelectedColor()
        {
            if (UIDevice.CurrentDevice.CheckSystemVersion(13,0))
            {
                nativeControl.SelectedSegmentTintColor = Element.SelectedBackgroundColor.ToUIColor();
            }
            else
            {
                var view = nativeControl.Subviews.FirstOrDefault();
                if (view != null)
                {
                    view.TintColor = Element.SelectedBackgroundColor.ToUIColor();
                }
            }
        }
        ...
        {
            var attr = new UITextAttributes();
            attr.TextColor = Element.TextColor.ToUIColor();
            nativeControl.SetTitleTextAttributes(attr, UIControlState.Normal);
        }
